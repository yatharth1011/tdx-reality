...
// ==========================================
const START_DATE = new Date(Date.UTC(2026, 6, 2)); // March 14, 2026
let TOTAL_DAYS = 37;
...
// ── Seed data (d=0 for all). Only used when no localStorage data exists yet. ──
const RAW_INITIAL_DATA = {
    "Organic Chemistry.csv": [
...
const DEFAULT_RESOURCES = Object.freeze([
    {name: "🧪 Organic Chemistry",    file: "Organic Chemistry.csv"},
    {name: "🧮 Math — Yellow Book",   file: "Math (Yellow Book).csv"},
    {name: "🧮 Math — Sameer Bansal", file: "Math (Sameer Bansal).csv"},
    {name: "🧮 Math — Pink Book",     file: "Math (Pink Book).csv"},
    {name: "⚗️ Physical Chemistry",   file: "Physical Chemistry.csv"},
    {name: "🧬 Inorganic Chemistry",  file: "Inorganic Chemistry.csv"},
    {name: "🧲 Physics (GQB)",        file: "Physics.csv"}
]);
const DEFAULT_RESOURCE_FILES = new Set(DEFAULT_RESOURCES.map(r => r.file));
const DEFAULT_RES_COLORS = Object.freeze({
    'Organic Chemistry.csv':   '#f5a623',
    'Math (Yellow Book).csv':  '#a78bfa',
    'Math (Sameer Bansal).csv':'#a78bfa',
    'Math (Pink Book).csv':    '#a78bfa',
    'Physical Chemistry.csv':  '#38bdf8',
    'Inorganic Chemistry.csv': '#34d399',
    'Physics.csv':             '#fb7185',
});
let RESOURCES_LIST = DEFAULT_RESOURCES.map(r => ({...r}));
...
const BADGE_DEFS = [
    ...
    // Subject — global
    {id:'org_25',          name:'Organic Chemist',   icon:'🧪', desc:'Completed 25% of Organic Chemistry.',          cat:'Subject', color:'#f5a623'},
    {id:'org_50',          name:'Organic Master',    icon:'🔬', desc:'Completed 50% of Organic Chemistry.',          cat:'Subject', color:'#f5a623'},
    {id:'pc_25',           name:'Physical Chemist',  icon:'⚗️', desc:'Completed 25% of Physical Chemistry.',         cat:'Subject', color:'#38bdf8'},
    {id:'pc_50',           name:'Physical Master',   icon:'💧', desc:'Completed 50% of Physical Chemistry.',         cat:'Subject', color:'#38bdf8'},
    {id:'ioc_25',          name:'Inorganic Chemist', icon:'🧬', desc:'Completed 25% of Inorganic Chemistry.',        cat:'Subject', color:'#34d399'},
    {id:'ioc_50',          name:'Inorganic Master',  icon:'🌿', desc:'Completed 50% of Inorganic Chemistry.',        cat:'Subject', color:'#34d399'},
    {id:'math_25',         name:'Mathematician',     icon:'📐', desc:'Completed 25% of Mathematics.',                cat:'Subject', color:'#a78bfa'},
    {id:'math_50',         name:'Math Maestro',      icon:'♾️', desc:'Completed 50% of Mathematics.',                cat:'Subject', color:'#a78bfa'},
    {id:'phy_25',          name:'Physicist',         icon:'🔭', desc:'Completed 25% of Physics.',                    cat:'Subject', color:'#fb7185'},
    {id:'phy_50',          name:'Physics Master',    icon:'🌌', desc:'Completed 50% of Physics.',                    cat:'Subject', color:'#fb7185'},
];
...
function _subjFilePct(files) {
    if (files.length === 1 && files[0] === 'Organic Chemistry.csv') {
        let {gD, gT} = _ocTrackerData();
        return gT > 0 ? (gD / gT) * 100 : 0;
    }
    let d = 0, t = 0;
    for (let fn of files) (appData.resources[fn] || []).forEach(r => { d += r.d + (r.s || 0); t += r.t; });
...
function evaluateGlobalBadges() {
    ...
    // Early Bird — Day 1 (only makes sense once)
    ...
    // Subject %
    let orgPct  = _subjFilePct(['Organic Chemistry.csv']);
    let pcPct   = _subjFilePct(['Physical Chemistry.csv']);
    let iocPct  = _subjFilePct(['Inorganic Chemistry.csv']);
    let mathPct = _subjFilePct(['Math (Yellow Book).csv','Math (Sameer Bansal).csv','Math (Pink Book).csv']);
    let phyPct  = _subjFilePct(['Physics.csv']);

    if (orgPct  >= 25) earned.add('org_25');  if (orgPct  >= 50) earned.add('org_50');
    if (pcPct   >= 25) earned.add('pc_25');   if (pcPct   >= 50) earned.add('pc_50');
    if (iocPct  >= 25) earned.add('ioc_25');  if (iocPct  >= 50) earned.add('ioc_50');
    if (mathPct >= 25) earned.add('math_25'); if (mathPct >= 50) earned.add('math_50');
    if (phyPct  >= 25) earned.add('phy_25');  if (phyPct  >= 50) earned.add('phy_50');
...
function loadData() {
    const LS_KEY  = 'tdxData';
    const LS_UNDO = 'tdxUndo';
    ...
    if (stored) {
        ...
        appData = {
            ...
            injectedTasks: parsed.injectedTasks || {}, ocCompleted: parsed.ocCompleted || {}
        };
        undoStack = parsed.undoStack || JSON.parse(localStorage.getItem(LS_UNDO) || '[]');
        // One-time migration: pull badges out of old separate key if present
        if (!appData.badges.length) {
            try { let old = JSON.parse(localStorage.getItem('tdx-badges') || '[]'); appData.badges = old; } catch {}
        }
    } else {
        appData = {
...
function saveData() {
    ...
    localStorage.setItem('tdxData', JSON.stringify({
        ...
        injectedTasks: appData.injectedTasks || {}, ocCompleted: appData.ocCompleted || {}
    }));
}
...
const SETTINGS_DEFAULTS = {
    ...
    physPriority:         [],
    // Never-skip selections
    noSkipGlobalSections: [],   // section names across all resources
    noSkipGlobalChapters: [],   // chapter names across all resources
    noSkipLocalSections:  [],   // [{file, chapter, section}, …]
    // Section priorities — null means "use data order" (populated lazily from resource data)
    sectionPriorities: {}
};
...
function applySettings(s) {
    ...
    // Load per-resource section priorities
    let sp = s.sectionPriorities || {};
    function _loadSP(file) {
        let v = sp[file];
        if (!v) return null;
        return Array.isArray(v) ? [...v] : v;   // preserve object reference for new format
    }
    ...
    if (btn) { btn.style.opacity = on ? '1' : '0.7'; btn.querySelector('i').textContent = on ? '🟫' : '⬛'; }
    applyResourceList(s);
}
...
function distributeTasks(queue, startDay, totalDays, todayAlreadyDone, todayOverrideQs) {
    ...
    for (let d=startDay; d<totalDays; d++) {
        ...
        if (d === startDay) {
            dailyTargets[d] = Math.max(0, Math.round(targetForToday) - todayAlreadyDone);
        } else {
            if (futureWeight > 0) {
                running += (targetForFuture / futureWeight) * w;
                let t = Math.round(running);
                dailyTargets[d] = t;
                running -= t;
            } else {
                dailyTargets[d] = 0;
...
function generateMasterSchedule() {
    let orgTasks = parseResource("Organic Chemistry.csv", null, null, "Even/Odd", null);
    // Split into g1 (priority exam chapters) and g2 (everything else),
    // then apply Obj→Multicorrect→Other cycle within each group — matches Python exactly.
    let g1 = orgTasks.filter(t => OC_GROUP1.includes(t.chapter));
    let g2 = orgTasks.filter(t => !OC_GROUP1.includes(t.chapter));
    // Sort g2 by OC_GROUP2 priority order (chapters not in the list go to end)
    g2.sort((a, b) => {
        let pA = OC_GROUP2.indexOf(a.chapter); if (pA === -1) pA = 999;
        let pB = OC_GROUP2.indexOf(b.chapter); if (pB === -1) pB = 999;
        return pA - pB;
    });
    function ocCycle(tasks) {
        return [
            ...tasks.filter(t => t.section.includes('Objective')),
            ...tasks.filter(t => t.section.includes('Multicorrect')),
            ...tasks.filter(t => !t.section.includes('Objective') && !t.section.includes('Multicorrect'))
        ];
    }
    let orgOrdered = [...ocCycle(g1), ...ocCycle(g2)];
    
    let mathQueueUnsorted = [
        ..._applySecPriority(parseResource("Math (Yellow Book).csv",   null, null, "Even/Odd", null), MATH_YB_SEC_PRIORITY),
        ..._applySecPriority(parseResource("Math (Sameer Bansal).csv", null, null, "Even/Odd", null), MATH_SB_SEC_PRIORITY),
        ..._applySecPriority(parseResource("Math (Pink Book).csv",     null, null, "ALL",      null), MATH_PB_SEC_PRIORITY)
    ];
    mathQueueUnsorted.sort((a, b) => {
        // If both are section-first, respect the Pass Index first
    ...
    let mathQueue = mathQueueUnsorted;
    
    let physChem = _applySecPriority(
        parseResource("Physical Chemistry.csv", null, null, "Even/Odd", PC_PRIORITY),
        PC_SECTION_PRIORITY
    );
    let ioc = _applySecPriority(
        parseResource("Inorganic Chemistry.csv", ["Level 1"], null, "Even/Odd", IOC_PRIORITY),
        IOC_SECTION_PRIORITY
    );
    let physics = _applySecPriority(
        parseResource("Physics.csv", null, null, "ALL", null),
        PHY_SECTION_PRIORITY
    );
    
    let allQueues = {
        "🧪 ORGANIC (SKM)": orgOrdered,
        "🧮 MATHS": mathQueue,
        "⚗️ PHYSICAL CHEM": physChem,
        "🧬 INORGANIC CHEM": ioc,
        "🧲 PHYSICS (GQB)": physics
    };

    // Extra user-added resources — grouped by their stored subject label
    let extraBySubj = {};
    RESOURCES_LIST.filter(r => !DEFAULT_RESOURCE_FILES.has(r.file)).forEach(r => {
        let custMeta = (appData.settings?.customResources || []).find(c => c.file === r.file) || {};
        let subj = custMeta.subj || '📦 EXTRA';
        let priList = (appData.settings?.customPriorities || {})[r.file] || null;
        if (!extraBySubj[subj]) extraBySubj[subj] = [];
        let custSecPri = (appData.settings?.sectionPriorities || {})[r.file] || [];
        extraBySubj[subj].push(..._applySecPriority(
            parseResource(r.file, null, null, "Even/Odd", priList), custSecPri));
    });
    for (let subj in extraBySubj) {
        if (extraBySubj[subj].length) {
            // Create the subject bucket if it does not exist yet, then safely append
            if (!allQueues[subj]) allQueues[subj] = [];
            allQueues[subj].push(...extraBySubj[subj]);
        }
    }
    
    // Apply never-skip: mark matching tasks as force_all before snapshot
    ...
    // qs count is subtracted from the queue so future days adjust accordingly.
    {
        ...
        todayInj.forEach(inj => {
            ...
            if (existing) {
                ...
                slot.unshift({ filename: inj.filename, chapter: inj.ch, section: inj.sec,
    ...
    // pending tasks proportionally so total assigned today stays ≈ N.
    {
        ...
        let todayTotalDone = todayComps.reduce((sum, c) => sum + c.done, 0);
...
function renderDay() {
    ...
    if (weight === 0.0 && comps.length === 0) {
        ...
        for (let subj in masterSchedules) {
            ...
            cardHtml += `</ul>
                <div style="padding:5px 13px 6px; border-top:1px solid var(--border); font-size:0.67rem; color:var(--text-subtle); display:flex; justify-content:space-between; font-family:'JetBrains Mono',monospace;">
        ...
        // Greedy 3-col balance (desktop) or single col (mobile)
        let isDesk = window.innerWidth >= 820;
        let finalHtml = '';

        if (isDesk) {
            // Sort tallest first so greedy fills evenly
        ...
        let tAssigned = tQs + tDone;
        let pct = tAssigned > 0 ? (tDone / tAssigned) * 100 : 100;
        let gradBar = pct >= 75 ? 'linear-gradient(90deg,#34d399,#6ee7b7)'
            : pct >= 40 ? 'linear-gradient(90deg,#fb923c,#fbbf24)'
            : 'linear-gradient(90deg,#f87171,#fb923c)';
        let summaryCard = `<div class="card summary-card" style="padding:11px 18px; display:flex; align-items:center; gap:20px;">
            <div style="font-family:'JetBrains Mono',monospace; font-size:1.6rem; font-weight:700; background:${gradBar}; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; min-width:52px;">${Math.round(pct)}%</div>
            <div style="flex:1;">
                <div style="display:flex; justify-content:space-between; font-size:0.67rem; color:var(--text-muted); margin-bottom:5px; font-family:'JetBrains Mono',monospace;">
                    <span>TOTAL COMBAT</span><span>${tDone} done · ${tAssigned} assigned</span>
                </div>
                <div class="progress-bar-bg" style="height:5px;"><div class="progress-bar-fill" style="width:${pct}%; background:${gradBar};"></div></div>
            </div>
        </div>`;

        // 'I want more' / AutoTarget capacity button
        if (viewingDayIdx === currentActualDayIdx && !!(appData.settings?.autoTarget)) {
            let _w = getDayWeight(currentActualDayIdx);
            let _capToday = Math.round(MAX_DAILY_CAPACITY * _w);
            let _hasCapacity = _capToday > tDone;
            if (tQs === 0 && tDone > 0) {
                // All assigned done — show assign UI
                let _btnLabel = _hasCapacity
                    ? `⚡ I want more… (${_capToday - tDone} Qs remaining capacity)`
                    : `⚡ Overdrive — assign extra combat`;
                finalHtml += `<button id="want-more-btn" onclick="openWantMore()">${_btnLabel}</button>`;
            } else if (tQs === 0 && _hasCapacity) {
                // No tasks logged yet but capacity exists — shouldn't normally show, but guard it
                finalHtml += `<button id="want-more-btn" onclick="openWantMore()">⚡ Assign today's combat (${_capToday - tDone} Qs available)</button>`;
            }
        }
        document.getElementById('schedule-content').innerHTML = finalHtml;

        // Update fixed combat bar
    ...
    if (oc_qs_str) {
        ...
        qs.forEach(q => {
            html += `<label style="display:flex; align-items:center; gap:6px; background:var(--surface); padding:8px; border:1px solid var(--border); border-radius:8px; cursor:pointer;">
                ...
                <span style="font-family:'JetBrains Mono',monospace; font-size:0.85rem; font-weight:bold;">${q}</span>
            </label>`;
        });
        html += `</div>`;
        ...
        if (sl.type === 'autotarget_boost') {
            ...
            let badgeRows = sl.badges.map(b => {
                ...
                let cntStr = (b.perAction && b.countNow > 1)
                    ? `<span style="font-size:0.75rem;color:var(--accent);font-family:'JetBrains Mono',monospace;font-weight:800;margin-left:5px;">×${b.countNow}</span>`
                    : '';
                let tag = isNew
        ...
        <div id="ae-qty-wrap" style="background:rgba(168,139,250,0.07);border:1.5px solid rgba(168,139,250,0.3);border-radius:12px;padding:12px;margin-top:10px;display:none;">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                <input type="number" id="ae-qty" min="1" placeholder="e.g. 25"
                    style="flex:1;min-width:80px;padding:10px;border-radius:8px;border:1px solid rgba(168,139,250,0.4);background:var(--bg);color:var(--text);font-family:'JetBrains Mono',monospace;font-size:1.1rem;text-align:center;">
...
function _aeChapChange() {
    ...
    listEl.innerHTML = tasks.map((t, i) => `
        <div id="ae-sec-${i}" onclick="_aeSelectSec(${i})"
            ...
            <span style="font-size:0.68rem;color:var(--text-muted);font-family:'JetBrains Mono',monospace;white-space:nowrap;">Rem: ${t.rem_raw}</span>
        </div>`).join('');
    _aeCtx.chap = chap;
    _aeCtx.tasks = tasks;
...
function renderCustomUI() {
    if(viewingDayIdx !== currentActualDayIdx) {
        document.getElementById('custom-sec-container').innerHTML = `<div style="color:var(--danger); padding:15px; text-align:center;">You can only log Custom Tasks for TODAY.</div>`;
        return;
    }
    
    let subjSel = document.getElementById('custom-subj');
    subjSel.innerHTML = '<option value="">-- Select Subject --</option>';
    for(let s in allQueuesClean) subjSel.innerHTML += `<option value="${s}">${s}</option>`;
    
    document.getElementById('custom-chap').style.display = 'none';
    document.getElementById('custom-sec-container').innerHTML = '';
}

function customSubjChanged() {
    let s = document.getElementById('custom-subj').value;
    let chapSel = document.getElementById('custom-chap');
    let secCon = document.getElementById('custom-sec-container');
    secCon.innerHTML = '';
    
    if(!s) { chapSel.style.display = 'none'; return; }
    
    let tasks = allQueuesClean[s];
    let chaps = [...new Set(tasks.map(t => t.chapter))];
    chapSel.innerHTML = '<option value="">-- Select Chapter --</option>';
    chaps.forEach(c => chapSel.innerHTML += `<option value="${c}">${c}</option>`);
    chapSel.style.display = 'block';
}

function customChapChanged() {
    let s = document.getElementById('custom-subj').value;
    let c = document.getElementById('custom-chap').value;
    let secCon = document.getElementById('custom-sec-container');
    secCon.innerHTML = '';
    ...
    let html = `<ul class="task-list" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">`;
    tasks.forEach(t => {
        ...
        html += `<li class="task-item">
                    <div class="task-info">
                        <div class="task-title">${t.section}</div>
                        <div class="task-meta">Rem: ${t.rem_raw}</div>
                    </div>
                    <div class="task-action" onclick="openLogModal('${s}', '${c}', '${t.section}', ${t.rem_raw}, '${t.filename}', 0, 1, '${oc_qs_str}')">LOG ▶</div>
                 </li>`;
    });
    html += `</ul>`;
    secCon.innerHTML = html;
}
...
function buildResHtml(res, compact) {
    ...
    for(let c in chaps) {
        ...
        html += `<div style="padding:7px 0; border-bottom:1px solid var(--border);">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:4px;">
                ...
                <span style="font-size:0.68rem; color:var(--text-muted); font-family:'JetBrains Mono',monospace; margin-left:8px; white-space:nowrap;">${d}/${t}</span>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
                ...
                <span style="font-size:0.65rem; color:var(--text-muted); font-family:'JetBrains Mono',monospace; min-width:28px; text-align:right;">${Math.round(p*100)}%</span>
            </div>`;
        ...
        if(rem > 0 && !compact) {
            chaps[c].secs.forEach(s => {
                ...
                html += `<div style="display:flex; align-items:center; gap:6px; margin-top:4px; padding-left:10px;">
                    ...
                    <span style="font-size:0.62rem; color:var(--text-subtle); font-family:'JetBrains Mono',monospace; min-width:22px; text-align:right;">${sr===0?'✔':'r:'+sr}</span>
                </div>`;
            });
        }
        html += `</div>`;
    ...
    html += `<div style="margin-top:10px; padding:8px 10px; background:rgba(255,255,255,0.03); border-radius:6px; display:flex; align-items:center; gap:10px;">
        <span style="font-size:0.65rem; color:var(--text-muted); font-family:'JetBrains Mono',monospace;">TOTAL</span>
        ...
        <span style="font-size:0.75rem; font-weight:700; background:${gradPct}; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace;">${gDone}/${gTotal}</span>
    </div>`;
    return html;
}
...
function renderVisualizer() {
    ...
    rdList.forEach((rd,ri)=>{
        ...
        if(span>0.26){
            ...
            labs+=`<text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle" dominant-baseline="central" font-size="${SZ*0.042}" style="pointer-events:none;">${em}</text>`;
        }
        // Chapter ring
    ...
    const svgHtml=`<svg width="${SZ}" height="${SZ}" viewBox="0 0 ${SZ} ${SZ}" style="display:block;overflow:visible;flex-shrink:0;">
        ...
        ${paths}
        ${labs}
        <circle cx="${cx}" cy="${cy}" r="${RC}" fill="rgba(0,0,0,0.65)" stroke="rgba(255,255,255,0.09)" stroke-width="1.5"/>
        <text x="${cx}" y="${cy-SZ*0.042}" text-anchor="middle" dominant-baseline="central" fill="url(#vLG)" font-family="'JetBrains Mono',monospace" font-size="${SZ*0.068}" font-weight="700">${campPctN}%</text>
        <text x="${cx}" y="${cy+SZ*0.018}" text-anchor="middle" dominant-baseline="central" fill="${campGC[0]}" font-family="'Bricolage Grotesque',sans-serif" font-size="${SZ*0.033}" font-weight="800" opacity="0.9">${rank.icon} ${rank.title.toUpperCase()}</text>
        <text x="${cx}" y="${cy+SZ*0.058}" text-anchor="middle" dominant-baseline="central" fill="rgba(255,255,255,0.28)" font-family="'JetBrains Mono',monospace" font-size="${SZ*0.024}">${campDone.toLocaleString()}/${campTotal.toLocaleString()}</text>
    </svg>`;
    ...
    const sectionHtml=rdList.map(rd=>{
        ...
        const chapRows=Object.entries(chaps).map(([ch,cv])=>{
            ...
            const chips=cv.secs.map(s=>{
                ...
                return `<div title="${s.sec}: ${s.d}/${s.t} done (${Math.round(sp*100)}%)" style="display:inline-flex;align-items:center;gap:2px;background:color-mix(in srgb,${col} 18%,transparent);border:1px solid color-mix(in srgb,${col} 44%,transparent);border-radius:4px;padding:2px 5px;font-size:0.5rem;font-family:'JetBrains Mono',monospace;color:${isBg?'rgba(255,255,255,0.28)':col};white-space:nowrap;flex-shrink:0;"><span style="width:5px;height:5px;border-radius:50%;background:${col};display:inline-block;flex-shrink:0;"></span>${lbl}${isDone?'✔':''}</div>`;
            }).join('');
            return `<div style="margin-bottom:5px;">
                <div style="display:flex;align-items:center;gap:5px;margin-bottom:3px;">
                    ...
                    <div style="font-size:0.52rem;color:${rc};font-family:'JetBrains Mono',monospace;flex-shrink:0;">${Math.round(cp*100)}%</div>
                </div>
    ...
    if(isDesk){
        ...
        const secCards = rdList.filter(rd=>rd.gT>0).map(rd=>{
            ...
            const chapRows=Object.entries(chaps).map(([ch,cv])=>{
                ...
                return `<div style="margin-bottom:5px;"><div style="display:flex;align-items:center;gap:5px;margin-bottom:3px;"><div style="font-size:0.58rem;font-weight:700;color:${rc};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0;">${ch}</div><div style="font-size:0.52rem;color:${rc};font-family:'JetBrains Mono',monospace;flex-shrink:0;">${Math.round(cp*100)}%</div></div><div style="display:flex;flex-wrap:wrap;gap:3px;">${chips}</div></div>`;
            }).join('');
    ...
    if (extra) {
        ...
        let weekBarHtml = weekDays.map(d => {
            ...
            return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;">
                <div style="font-size:0.55rem;color:var(--accent);font-family:'JetBrains Mono',monospace;">${d.total>0?d.total:''}</div>
        ...
        let weekHtml = `
        <div style="font-family:'JetBrains Mono',monospace; color:var(--accent); font-size:0.65rem; letter-spacing:2px; text-transform:uppercase; margin-bottom:10px;">Weekly Report</div>
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:14px; padding:12px; margin-bottom:14px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-top:8px;border-top:1px solid var(--border);padding-top:8px;">
                <div style="text-align:center;">
                    <div style="font-size:1.1rem;font-weight:800;color:var(--accent);font-family:'JetBrains Mono',monospace;">${weekTotal}</div>
                    <div style="font-size:0.55rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Total Qs</div>
                </div>
                <div style="text-align:center;">
                    <div style="font-size:1.1rem;font-weight:800;color:var(--success);font-family:'JetBrains Mono',monospace;">${weekBest}</div>
                    <div style="font-size:0.55rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Best Day</div>
                </div>
                <div style="text-align:center;">
                    <div style="font-size:1.1rem;font-weight:800;color:var(--warning);font-family:'JetBrains Mono',monospace;">${weekActive}/7</div>
        ...
        let badgesHtml = `<div style="font-family:'JetBrains Mono',monospace; color:var(--accent); font-size:0.65rem; letter-spacing:2px; text-transform:uppercase; margin-bottom:10px; margin-top:16px;">Achievements <span style="color:var(--text-muted);">${earnedSet.size}/${BADGE_DEFS.length}</span></div>`;
        cats.forEach(cat => {
            ...
            catBadges.forEach(b => {
                ...
                let cntChip = (earned && cnt > 1) ? `<div style="margin-top:2px;font-size:0.63rem;font-weight:800;color:var(--bc,var(--accent));font-family:'JetBrains Mono',monospace;letter-spacing:0.5px;">×${cnt}</div>` : '';
        ...
        let logHtml = `<div style="font-family:'JetBrains Mono',monospace; color:var(--accent); font-size:0.65rem; letter-spacing:2px; text-transform:uppercase; margin:14px 0 10px;">Activity Log</div>`;
            ...
            sortedDays.slice(0, 20).forEach(ds => {
                ...
                logHtml += `<div style="margin-bottom:10px; padding:10px; background:var(--surface); border:1px solid ${isToday?'var(--border-accent)':'var(--border)'}; border-radius:6px;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                        <span style="font-size:0.72rem; font-family:'JetBrains Mono',monospace; color:${isToday?'var(--accent)':'var(--text-muted)'};">${ds}${isToday?' ◄ TODAY':''}</span>
...
// ==========================================
// EDITOR UI
// Maps select value → { subj key, color }
const SUBJ_SELECT_MAP = {
    physics: { subj: '🧲 PHYSICS (GQB)',  color: '#fb7185' },
    math:    { subj: '🧮 MATHS',           color: '#a78bfa' },
    pc:      { subj: '⚗️ PHYSICAL CHEM',   color: '#38bdf8' },
    ioc:     { subj: '🧬 INORGANIC CHEM',  color: '#34d399' },
    oc:      { subj: '🧪 ORGANIC (SKM)',    color: '#f5a623' },
    extra:   { subj: '📦 EXTRA',           color: '#94a3b8' },
};
...
function renderEditorResourceList() {
    ...
    custom.forEach(r => {
        html += `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);">
            ...
            <span style="flex:1;font-size:0.8rem;font-weight:600;">${r.name} <span style="font-size:0.65rem;color:var(--text-muted);">(custom)</span></span>
        </div>`;
    });
...
function openAddResourceForm() {
    ...
    document.getElementById('ares-name').value = '';
    document.getElementById('ares-subj').value = 'physics';
}
...
function editorAddResource() {
    let name    = document.getElementById('ares-name').value.trim();
    let subjKey = document.getElementById('ares-subj').value;
    if (!name) { alert('Enter a display name.'); return; }
    let { subj, color } = SUBJ_SELECT_MAP[subjKey] || SUBJ_SELECT_MAP.extra;
    ...
    if (!appData.resources[file]) appData.resources[file] = [];
...
function editorDeleteSection(file, ch, sec) {
    ...
    appData.resources[file] = (appData.resources[file] || []).filter(r => !(r.ch === ch && r.sec === sec));
...
function editorAddChapter(file) {
    ...
    rows.push({ ch, sec: 'Section A (Objective)', d: 0, t: 0, s: 0 });
    appData.resources[file] = rows;
    saveData(); needsRebuild = true; renderEditor();
    let grp = document.getElementById(`edgrp_${encodeURIComponent(ch)}`);
    if (grp) grp.classList.add('open');
...
function openSettings() {
    ...
    let pc  = s.pcPriority     || d.pcPriority;
    let ioc = s.iocPriority    || d.iocPriority;
    let allMathChapters = [...new Set([
        ...(appData.resources["Math (Yellow Book).csv"] || []).map(r => r.ch),
        ...(appData.resources["Math (Sameer Bansal).csv"] || []).map(r => r.ch),
        ...(appData.resources["Math (Pink Book).csv"] || []).map(r => r.ch)
    ])];
    let savedMath = s.mathPriority || d.mathPriority;
    let mathMerged = [...savedMath.filter(c => allMathChapters.includes(c)), ...allMathChapters.filter(c => !savedMath.includes(c))];
    let oc1 = s.ocGroup1 || d.ocGroup1;
    // Build oc2: all OC_ASSIGNMENTS chapters not in oc1, merged with saved order
    let allOcChapters = Object.keys(OC_ASSIGNMENTS).filter(ch => !oc1.includes(ch));
    let savedOc2 = s.ocGroup2 || d.ocGroup2;
    let oc2 = [...savedOc2.filter(c => allOcChapters.includes(c)), ...allOcChapters.filter(c => !savedOc2.includes(c))];
    let sp  = s.sectionPriorities || {};
    ...
    // ── Build custom resource priority sections ──
    let customPriSections = '';
    (s.customResources || []).forEach(r => {
        let chapters = [...new Set((appData.resources[r.file] || []).map(row => row.ch))];
        if (!chapters.length) return;
        // Ensure saved list matches current chapters (add new, drop removed)
        let saved = (s.customPriorities || {})[r.file] || chapters;
        let merged = [...saved.filter(c => chapters.includes(c)),
                      ...chapters.filter(c => !saved.includes(c))];
        let listId  = 'cust_' + r.file.replace(/[^a-zA-Z0-9]/g,'_');
        let secHtml = _buildSecPriSubGroup(r.file, 'sp_' + listId, sp[r.file] || null);
        customPriSections += _buildPriGroup(
            '📦', r.name + ' — Chapter & Section Priority',
            'Drag chapters to reorder. Expand section sub-group to set section order.',
            _buildDragList(listId, merged) + secHtml,
            'custom_' + listId,
            false
        );
    });
    ...
    document.getElementById('settings-body').innerHTML = `
        ...
        ${_buildPriGroup('⚙️', 'General', null, `
            <div class="sett-row">
                <div>
                    <div class="sett-row-label">AMOLED Mode</div>
                    <div class="sett-row-desc">Pure black background — saves battery on OLED screens.</div>
                </div>
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;flex-shrink:0;">
                    <input type="checkbox" id="sett-amoled" ${amoledOn?'checked':''} style="width:18px;height:18px;accent-color:var(--accent);cursor:pointer;">
                </label>
            </div>
        ...
        ${_buildPriGroup('⚗️', 'Physical Chemistry — Chapter Priority',
            'Drag to reorder. Top = highest priority = scheduled first.',
            _buildDragList('pc', pc)
            + _buildSecPriSubGroup('Physical Chemistry.csv', 'sp_pc', sp['Physical Chemistry.csv'] || null),
            'pc', false)}

        ${_buildPriGroup('🧬', 'Inorganic Chemistry — Chapter Priority',
            'Drag to reorder. Top = highest priority = scheduled first.',
            _buildDragList('ioc', ioc)
            + _buildSecPriSubGroup('Inorganic Chemistry.csv', 'sp_ioc', sp['Inorganic Chemistry.csv'] || null),
            'ioc', false)}

        ${_buildPriGroup('🧮', 'Mathematics — Global Chapter Priority',
            'Drag to reorder. Affects all Math books globally.',
            _buildDragList('math', mathMerged)
            + _buildSecPriSubGroup('Math (Yellow Book).csv',   'sp_math_yb', sp['Math (Yellow Book).csv']   || null)
            + _buildSecPriSubGroup('Math (Sameer Bansal).csv', 'sp_math_sb', sp['Math (Sameer Bansal).csv'] || null)
            + _buildSecPriSubGroup('Math (Pink Book).csv',     'sp_math_pb', sp['Math (Pink Book).csv']     || null),
            'math', false)}

        ${_buildPriGroup('🧲', 'Physics (GQB) — Chapter & Section Priority',
            'Drag chapters to reorder; expand section group to set section order.',
            (() => {
                let physChaps = [...new Set((appData.resources['Physics.csv'] || []).map(r => r.ch))];
                let savedPhys  = s.physPriority || [];
                let mergedPhys = [...savedPhys.filter(c => physChaps.includes(c)),
                                  ...physChaps.filter(c => !savedPhys.includes(c))];
                let chHtml = physChaps.length
                    ? _buildDragList('phy', mergedPhys)
                    : '<div class="pri-group-hint" style="color:var(--text-subtle);">No Physics data loaded.</div>';
                let secHtml = _buildSecPriSubGroup('Physics.csv', 'sp_phy', sp['Physics.csv'] || null);
                return chHtml + secHtml;
            })(),
            'phy', false)}

        ${_buildPriGroup('🧪', 'Organic Chemistry — Priority Group 1',
            'These chapters are cycled first (Obj→MC→Other), then the rest.',
            _buildDragList('oc1', oc1),
            'oc1', false)}

        ${_buildPriGroup('🧪', 'Organic Chemistry — Priority Group 2',
            'Order of Group 2 chapters (scheduled after Group 1). Drag to reorder.',
            _buildDragList('oc2', oc2),
            'oc2', false)}
...
function saveSettings() {
    if (!appData.settings) appData.settings = {};
    let s = appData.settings;
    s.amoled      = document.getElementById('sett-amoled').checked;
    ...
    s.pcPriority       = _readDragList('pc')  || s.pcPriority;
    s.iocPriority      = _readDragList('ioc') || s.iocPriority;
    s.mathPriority     = _readDragList('math') || s.mathPriority;
    s.ocGroup1         = _readDragList('oc1') || s.ocGroup1;
    s.ocGroup2         = _readDragList('oc2') || s.ocGroup2;
    // Never-skip settings
    ...
    (s.customResources || []).forEach(r => {
        let listId = 'cust_' + r.file.replace(/[^a-zA-Z0-9]/g,'_');
        let result = _readDragList(listId);
        if (result) s.customPriorities[r.file] = result;
    });

    // Persist section priorities
    if (!s.sectionPriorities) s.sectionPriorities = {};
    [['Physical Chemistry.csv',   'sp_pc'],
     ['Inorganic Chemistry.csv',  'sp_ioc'],
     ['Math (Yellow Book).csv',   'sp_math_yb'],
     ['Math (Sameer Bansal).csv', 'sp_math_sb'],
     ['Math (Pink Book).csv',     'sp_math_pb'],
     ['Physics.csv',              'sp_phy']].forEach(([file, lid]) => {
        const wid = lid.replace(/[^a-zA-Z0-9]/g, '_');
        const cfg = _readSecPassConfig(wid);
        if (cfg) s.sectionPriorities[file] = cfg;
    });
    (s.customResources || []).forEach(r => {
        const listId = 'sp_cust_' + r.file.replace(/[^a-zA-Z0-9]/g, '_');
        const wid    = listId.replace(/[^a-zA-Z0-9]/g, '_');
        const cfg    = _readSecPassConfig(wid);
        if (cfg) s.sectionPriorities[r.file] = cfg;
    });
    applySettings(s); saveData(); needsRebuild = true; closeSettings(); renderDay();
}
...
function renderDayStrip() {
    ...
    for (let d = start; d <= end; d++) {
        ...
        ">
            ...
            <div style="font-size:0.88rem; font-weight:700; font-family:'JetBrains Mono',monospace; line-height:1.2;">${dayNum}</div>
...
function renderRightPanel() {
    ...
    RESOURCES_LIST.forEach(r => {
        ...
        const gap = (circ - p * circ).toFixed(2);
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:5px;padding:10px 4px 8px;
            ...
            <svg width="${C*2}" height="${C*2}" viewBox="0 0 ${C*2} ${C*2}" style="overflow:visible;display:block;">
                <circle cx="${C}" cy="${C}" r="${R}" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="${STROKE}"/>
                ...
                <text x="${C}" y="${C}" text-anchor="middle" dominant-baseline="central"
                    fill="${rc}" font-family="'JetBrains Mono',monospace" font-size="9" font-weight="700">${pct}%</text>
            </svg>
            ...
            <div style="font-size:0.55rem;color:color-mix(in srgb,${rc} 80%,transparent);font-family:'JetBrains Mono',monospace;">${d}/${t}</div>
        </div>`);
    });
    ...
    let recentHtml = allComps.slice(0, 7).map(c => `
        <div style="padding:6px 0; border-bottom:1px solid var(--border);">
            <div style="display:flex; justify-content:space-between;">
                <span style="font-size:0.65rem; color:var(--text-muted); font-family:'JetBrains Mono',monospace;">${c.date}</span>
    ...
    panel.innerHTML = `
        ...
        <div style="padding:20px 14px 16px; background:color-mix(in srgb,${todayGrad.includes('34d399')?'#34d399':todayGrad.includes('fb923c')?'#fb923c':'#f87171'} 8%,var(--surface-light)); border:1px solid var(--border); border-radius:24px; margin-bottom:13px; text-align:center; animation:springUp var(--dur-lg) var(--spring) both;">
            <div style="font-size:3.8rem; font-weight:800; background:${todayGrad}; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; line-height:1; animation:heroCount var(--dur-lg) var(--spring) both;">${todayPct}%</div>
            ...
            <div style="font-size:0.63rem; color:var(--text-subtle); margin-top:5px; font-family:'JetBrains Mono',monospace;">${todayDone} done &nbsp;·&nbsp; ${todayAssigned} assigned</div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px; margin-bottom:14px;">
            <div style="background:color-mix(in srgb,#fb923c 8%,var(--surface-light)); border:1px solid var(--border); border-radius:20px; padding:14px 8px; text-align:center; animation:springUp var(--dur-lg) var(--spring) 60ms both;">
                <div style="font-size:2.4rem; font-weight:800; background:linear-gradient(135deg,#fb923c,#fbbf24); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; animation:heroCount var(--dur-lg) var(--spring) 80ms both;">${strk}</div>
            ...
            <div style="background:color-mix(in srgb,#f5a623 8%,var(--surface-light)); border:1px solid var(--border); border-radius:20px; padding:14px 8px; text-align:center; animation:springUp var(--dur-lg) var(--spring) 120ms both;">
                <div style="font-size:2.4rem; font-weight:800; background:linear-gradient(135deg,#f5a623,#fbbf24); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; animation:heroCount var(--dur-lg) var(--spring) 140ms both;">${daysLeft}</div>
            ...
            <div style="background:color-mix(in srgb,#a78bfa 8%,var(--surface-light)); border:1px solid var(--border); border-radius:20px; padding:14px 8px; text-align:center; animation:springUp var(--dur-lg) var(--spring) 180ms both;">
                <div style="font-size:2.4rem; font-weight:800; background:linear-gradient(135deg,#a78bfa,#818cf8); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; animation:heroCount var(--dur-lg) var(--spring) 200ms both;">${getPersonalBest().best||0}</div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:14px;">
            <div style="background:color-mix(in srgb,#34d399 8%,var(--surface-light)); border:1px solid var(--border); border-radius:20px; padding:14px 8px; text-align:center; animation:springUp var(--dur-lg) var(--spring) 240ms both;">
                <div style="font-size:2rem; font-weight:800; background:linear-gradient(135deg,#34d399,#6ee7b7); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; animation:heroCount var(--dur-lg) var(--spring) 260ms both;">${totalEffRem.toLocaleString()}</div>
            ...
            <div style="background:color-mix(in srgb,#818cf8 8%,var(--surface-light)); border:1px solid var(--border); border-radius:20px; padding:14px 8px; text-align:center; animation:springUp var(--dur-lg) var(--spring) 300ms both;">
                <div style="font-size:2rem; font-weight:800; background:linear-gradient(135deg,#818cf8,#a78bfa); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace; animation:heroCount var(--dur-lg) var(--spring) 320ms both;">${idealDaily}</div>
        ...
        <div style="background:var(--surface-light); border:1px solid var(--border); border-radius:var(--r); padding:11px 12px; margin-bottom:13px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:7px;">
                ...
                <span style="font-size:0.85rem; font-weight:700; background:${campGrad}; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-family:'JetBrains Mono',monospace;">${Math.round(campDonePct)}%</span>
            </div>
            <div class="progress-bar-bg" style="height:6px; background:rgba(255,255,255,0.04); position:relative; overflow:hidden;">
                <div class="progress-bar-fill" style="width:${campDonePct}%; background:${campGrad}; position:absolute; left:0; top:0; height:100%;"></div>
                <div style="width:${campSkipPct}%; background:rgba(180,180,180,0.3); position:absolute; left:${campDonePct}%; top:0; height:100%;"></div>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:0.62rem; color:var(--text-subtle); margin-top:5px; font-family:'JetBrains Mono',monospace;">
...
function init() {
    ...
    // Apply all persisted settings (AMOLED, priorities, daily cap, resource list)
    ...
    let today = new Date();
    // Use local midnight to prevent TZ shifts overriding the day logically
    let shifted = new Date(today.getTime() - 4 * 3600000);
    let todayUTC = Date.UTC(shifted.getFullYear(), shifted.getMonth(), shifted.getDate());
    
    if (todayUTC <= START_DATE.getTime() + (TOTAL_DAYS*86400000) && todayUTC >= START_DATE.getTime()) {
        currentActualDayIdx = Math.floor((todayUTC - START_DATE.getTime()) / 86400000);
    } else if (todayUTC > START_DATE.getTime()) {
        document.body.innerHTML = `<div style="text-align:center; padding:50px 20px;"><h2>🏁 SCHEDULE COMPLETE</h2><p>The war is over. Bhagwan sab theek karega.</p></div>`;
        return;
    } else {
...
function updateDropletTradeoffs() {
    ...
    activeSubjs.forEach(s => {
        ...
        html += `<div style="display:grid; grid-template-columns:2fr 1fr 1.2fr; gap:8px; font-size:0.8rem; margin-bottom:4px; align-items:center;">
            ...
            <span style="text-align:right; font-family:'JetBrains Mono',monospace;">${newToday} <span style="font-size:0.55rem; display:inline-block; min-width:20px; text-align:left;">(${diffStr})</span></span>
            <span style="text-align:right; font-family:'JetBrains Mono',monospace;">${newFutureAvg}</span>
        </div>`;
    });
    ...
    activeSubjs.forEach(subj => {
        ...
        html += `<div style="margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:4px; font-size:0.8rem;">
                ...
                <span id="${id}-val" style="font-family:'JetBrains Mono',monospace; color:var(--text-muted);">${w}</span>
            </div>
        ...
        for (let sec in OC_ASSIGNMENTS[ch]) {
            ...
            qs.forEach(q => {
                ...
                html += `<label style="display:flex; justify-content:center; align-items:center; background:var(--bg); border:1px solid var(--border); border-radius:6px; padding:6px; cursor:pointer;">
                            ...
                            <span style="font-family:'JetBrains Mono',monospace; font-size:0.75rem;">${q}</span>
                         </label>`;
            });
            html += `</div>`;
        }
