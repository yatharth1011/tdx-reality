...
function applyPreset() {
    ...
    document.getElementById('ares-subj').value = parts[1];
}
...
function generateMasterSchedule() {
    ...
    const getPriList = (f) => {
        ...
        return (appData.settings?.customPriorities || {})[f] || null;
    };
    ...
    const getSecPri = (f) => {
        ...
        return (appData.settings?.sectionPriorities || {})[f] || [];
    };
...
function editorAddResource() {
    ...
    if (!appData.resources[file]) {
        appData.resources[file] = PRESET_DATA[file] ? JSON.parse(JSON.stringify(PRESET_DATA[file])) : [];
    }
...
function openSettings() {
    ...
    let presetFiles = ['Neeraj Kumar JA.csv', 'IOC VK Jaiswal.csv', 'OC SKM.csv', 'Math (Yellow Book).csv', 'Math (Sameer Bansal).csv', 'Math (Pink Book).csv', 'Physics (GQB).csv'];
    (s.customResources || []).forEach(r => {
        if (presetFiles.includes(r.file)) return;
